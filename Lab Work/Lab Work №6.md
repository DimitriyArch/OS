...
| `echo` | Команда `echo` використовується для виведення тексту або значення змінної в термінал. Вона часто застосовується в скриптах для відображення повідомлень або результатів виконання команд.|
| `cat` | Команда `cat` призначена для перегляду вмісту файлів. Вона виводить текст файлу прямо в термінал і також може використовуватися для об’єднання кількох файлів або створення нового файлу.|
| `find` | Команда `find` використовується для пошуку файлів і каталогів у файловій системі за різними критеріями, наприклад за ім’ям, типом, розміром або датою зміни.|
| `tr` | Команда `tr` призначена для перетворення символів у тексті. Вона може замінювати одні символи іншими, видаляти певні символи або змінювати регістр літер у тексті.|
| `ls -l` | Команда `ls -l` використовується для перегляду списку файлів і каталогів у поточній директорії у детальному форматі. Вона показує права доступу, кількість посилань, власника, розмір файлу та дату останньої зміни.|
| `cut -d: -f1` | Команда `cut -d: -f1` використовується для вирізання частини тексту з рядків файлу. Параметр `-d:` задає роздільник (у цьому випадку двокрапку), а `-f1` означає, що потрібно вивести перше поле з кожного рядка.|
| `head` | Команда `head` показує перші рядки файлу. За замовчуванням вона виводить перші десять рядків тексту, що зручно для швидкого перегляду початку великого файлу.|
| `tail` | Команда `tail` показує останні рядки файлу. За замовчуванням також виводиться десять рядків, що дозволяє швидко побачити кінець файлу або нові записи в логах.|
| `grep` | Команда `grep` використовується для пошуку рядків у тексті, які відповідають заданому шаблону або слову. Вона часто застосовується для фільтрації результатів інших команд.|
| `egrep` | Команда `egrep` є розширеною версією grep і дозволяє використовувати складніші регулярні вирази для пошуку тексту.|
