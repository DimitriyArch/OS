...
Команди `arch` та `lscpu` мають різні призначення: `arch` показує лише архітектуру процесора (наприклад, x86_64), тоді як `lscpu` виводить детальну інформацію про процесор, включаючи кількість ядер, потоки, кеші, підтримувані інструкції та інші характеристики.

### 2. Якою командою можна отримати інформацію про стан використання RAM поточною системою?

Інформацію про стан використання RAM у поточній системі можна отримати командою `free -h` або `cat /proc/meminfo`, які показують загальний обсяг пам’яті, доступну, використану та кешовану.

### 3. Яким чином у скриптах можна опрацьовувати змінні та створювати розгалужені та циклічні сценарії?

У скриптах змінні створюються просто, наприклад, `VAR=value`, звернення до них відбувається через `$VAR`. Для розгалужень використовуються конструкції `if…then…else…fi`, а для циклів – `for…do…done` або `while…do…done`. Це дозволяє автоматизувати логіку та виконувати дії залежно від умов.

### 4. Які команди для перегляду стану підключення периферійних пристроїв можна використати в терміналі? 

Для перегляду стану підключення периферійних пристроїв у терміналі можна використати команди: `lsusb` (USB-пристрої), `lspci` (PCI-пристрої), `lsblk` (блокові пристрої), `dmesg | grep` для перевірки логів підключень.

### 5. Які можливості застунку gparted?

GParted – це графічний інструмент для роботи з розділами дисків. Він дозволяє створювати, видаляти, змінювати розмір, форматувати розділи, змінювати файлову систему, встановлювати прапорці розділів (boot, hidden), перевіряти та відновлювати файлові системи, а також управляти таблицею розділів диска (MBR або GPT).
